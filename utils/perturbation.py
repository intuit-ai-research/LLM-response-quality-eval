...
class Paraphrasing(Perturbation):
    """
    The prompt is perturbed by being paraphrased
    """

    def __init__(self, n: int, llm: LLM) -> None:
        self.n = n
        self.llm = llm
    ...
    def paraphrase(self, content: str, temperature=0.7) -> list:

        model_config = {"temperature": temperature}

        out = self.llm.generate([{'role': 'user', 'content': '"""\n' + content + '\n"""\n' + self.cmd}], **model_config)
        if out is None:
            return None
        t0 = out.find('{')
        if t0 < 0:
    ...
    def perturb(self, messages: list, temperature: float) -> list:
        ...
        paraphrased = self.paraphrase(orig, temperature)
        perturbed = []
