from utils.perturbation import TemperaturePerturbation, DummyToken, RandSysMsg, Paraphrasing
import numpy as np
import unittest
...
class TestPerturbation(unittest.TestCase):
    ...
    def test_prompt_perturbation(self):
        for _Perturbation in [DummyToken, RandSysMsg]:
        ...
        llm = LLM_OpenAI("gpt-35-turbo-0301")
        perturbation = Paraphrasing(n=self.n, llm = llm)
        perturbed = perturbation.perturb(self.messages, self.temperature)
        print(perturbation)
        print(perturbed)
        self.assertTrue(isinstance(perturbed, list))
